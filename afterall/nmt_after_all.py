...
def init_params(options):
    ...
    params['Wemb_dec'] = norm_weight(options['n_words'], options['dim_word'] / 2)
    # encoder
...
def build_model(tparams, options):
    ...
    emb = emb.reshape([n_timesteps_trg, n_samples, options['dim_word'] / 2])
    ...
    topic = topic.reshape([n_timesteps_trg, n_samples, options['dim_word'] / 2])
    
    # concatenate word embedding(target) & topic embedding
...
def build_sampler(tparams, options, trng, use_noise):
    ...
    # if it's the first word, emb should be all zero (add topic)
    # concatenate word embedding & topic embedding
    zero = tensor.alloc(0., 1, tparams['Wemb_dec'].shape[1])
    gen = tparams['Wemb_dec'][y]
...
def prepare_z(x, y, worddicts):
    z = []
    for i in range(len(x)):
        seqx = tuple(x[i])
        if seqx not in worddicts[1]:
           print "not found seqx in worddicts[1]!"
           exit() 
