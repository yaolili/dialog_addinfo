...
def init_params(options):
    params = OrderedDict()
    # embedding
    params['Wemb'] = norm_weight(options['n_words_src'], options['dim_word'] / 2)
...
def build_model(tparams, options):
    ...
    emb = emb.reshape([n_timesteps, n_samples, options['dim_word'] / 2])
    
    # topic embedding 
    topic = tparams['Wemb'][z.flatten()]
    topic = topic.reshape([n_timesteps, n_samples, options['dim_word'] / 2])
    
    # concatenate word embedding & topic embedding
    emb = concatenate([emb, topic], axis=2)
