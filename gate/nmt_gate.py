...
def param_init_gru(options, params, prefix='gru', nin=None, dim=None):
    ...
    # add topic gate
    Wt = norm_weight(nin, dim)
    params[_p(prefix, 'Wt')] = Wt
    params[_p(prefix, 'bt')] = numpy.zeros((dim,)).astype('float32')
...
def gru_layer(tparams, state_below, z, options, prefix='gru', mask=None,
    ...
    # topic gate
    z_g = tensor.dot(z, tparams[_p(prefix, 'Wt')]) + \
        tparams[_p(prefix, 'bt')]

    # step function to be used by scan
    # arguments    | sequences |outputs-info| non-seqs
    def _step_slice(m_, x_, xx_, z_g, h_,          U, Ux):
    ...
    seqs = [mask, state_below_, state_belowx, z_g]
